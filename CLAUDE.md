...
No build system, no package.json, no tests, no linter. Plain `<script>` tags and a stdlib-only Python server. UI strings are Indonesian.
...
- Dev on Windows: `npm install`, build sidecar once, then `npx tauri dev`. Sidecar shutdown is two-layered: Rust kills the whole process tree with `taskkill /F /T` (PyInstaller onefile spawns a same-named child, so plain `kill` orphans it) on `ExitRequested`/`Exit`, and server.py runs a stdin-EOF watchdog (`_parent_watchdog`) that `os._exit(0)`s when the Tauri parent dies.
