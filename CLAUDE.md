...
The page is an app shell: sticky topbar + left sidebar (`js/nav.js`, hash router `#/dashboard|konversi|riwayat|pengaturan|tentang`) + `<main class="shell">` holding five `<section class="view" id="view-*">`. The whole conversion flow lives untouched inside `view-konversi`; `view-tentang` is a static about page rendered by `main.js initAboutPage()` (version via Tauri `app.getVersion()` / "Web — server.py"). On narrow screens the sidebar becomes off-canvas (hamburger `#nav-toggle`, scrim, `body.nav-open`).
...
The same codebase also ships as a desktop app via Tauri v2 (`src-tauri/`). Architecture: **dev mode** (`npm run dev`) serves the working tree directly via `scripts/dev_server.py` (`build.devUrl` = `http://127.0.0.1:1430`, launched by `beforeDevCommand`) with SSE live-reload — saving `index.html`/`css/*`/`js/*` reloads the webview automatically. **Production** (`npm run build`) loads static assets from `src-tauri/dist/` (synced copy via `scripts/sync_dist.py`, run by `beforeBuildCommand`).

- Sidecar build: `python scripts/build_sidecar.py` (needs `pip install -r requirements.txt pyinstaller`) → writes `src-tauri/binaries/valency-server-<rust-triple>[.exe]`. Required before `tauri dev`/`tauri build`; `externalBin` in `tauri.conf.json`.
...
- Dev on Windows: `npm install`, `npm run sidecar` (build sidecar once), then `npm run dev`. Sidecar shutdown is two-layered: Rust kills the process (whole tree via `taskkill /F /T` on Windows) on `ExitRequested`/`Exit`, and server.py's `_parent_watchdog` cleans up the PyInstaller child — stdin-EOF on Windows, `getppid() == 1` orphan polling on Unix (the inherited stdin fd never sees EOF when SIGKILLed). HTML5 drag-drop needs `dragDropEnabled: false` in tauri.conf.json (default true makes Tauri swallow native drops in WKWebView before they reach JS).
...
- Versioning: keep the version in sync across `package.json`, `src-tauri/tauri.conf.json`, and `src-tauri/Cargo.toml` (Tauri `app.getVersion()` reads `tauri.conf.json`).
